...
(function () {
    ...
    const BASE = 'https://www.douban.com'
    ...
    async function fetchReasons() {
        ...
        const url = `https://m.douban.com/rexxar/api/v2/group/${state.groupId}/report_reasons`
        const res = await fetch(url, { credentials: 'include' })
        if (!res.ok) throw new Error(`加载举报原因失败：${res.status}`)
    ...
    async function reportOne(commentId, reason) {
        ...
        const res = await fetch(`${BASE}/j/group/${state.groupId}/member_report`, {
            method: 'POST',
            credentials: 'include',
            headers: {
